...
(function () {
  var count = 0;
  var cv = document.getElementById('base-div');
  var ps = cv.appendChild(document.createElement('p'));
  var btn = cv.appendChild(document.createElement('button'));
  btn.addEventListener('click', function (e) {
    ps.innerText = String(count++);
  });
  var canvas = (0, _d.select)('body').append('canvas').attr('width', 200).attr('height', 200).node().getContext('2d');
  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;

  try {
    for (var _iterator = (0, _d.range)(200)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var x = _step.value;
      canvas.beginPath();
      canvas.moveTo(x, 0);
      canvas.strokeStyle = (0, _d.interpolateCubehelixDefault)(x / 200);
      canvas.lineTo(x, 200);
      canvas.stroke();
    }
  } catch (err) {
...
},{"d3":"../node_modules/d3/index.js"}]},{},["index.ts"], null)
